...
void remove_link(node_link_list_t *link, char *room)
{
    ...
    if (link == NULL)
        return;
    if (my_strcmp(link->link->room, room) == 0) {
        link = link->next;
        free(tmp);
        return;
    }
    if (link->next == NULL)
        return;
    while (link->next->next != NULL && my_strcmp(link->link->room, room) != 0)
        link = link->next;
    if (my_strcmp(link->next->link->room, room) != 0
    && my_strcmp(link->link->room, room) != 0)
        return;
    tmp = link->next;
...
int is_link(node_t *node, node_t *node_link)
{
    for (node_link_list_t *tmp = node->link_list; tmp; tmp = tmp->next)
        if (my_strcmp(tmp->link->room, node_link->room) == 0)
            return (1);
    return (0);
}
...
node_t *get_node(node_link_list_t *node_list, char *room)
{
    for (node_link_list_t *tmp = node_list; tmp; tmp = tmp->next)
        if (my_strcmp(room, tmp->link->room) == 0)
            return (tmp->link);
    return (NULL);
}
