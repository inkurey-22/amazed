...
static int get_size(robot_t *robot)
{
    ...
    if (robot->index && my_strcmp(robot->index, "links") == 0)
        size -= 9;
    for (int i = 0; room[i] != 0; i++) {
        if (start && my_strcmp(room[i][0], start) == 0)
            size += 8;
        if (end && my_strcmp(room[i][0], end) == 0)
            size += 6;
...
static int add_room(robot_t *robot, char *buffer, char ***room, int i)
{
    ...
    for (int j = 0; room[j] != 0; j++) {
        if (start && my_strcmp(room[j][0], robot->room_start[0]) == 0)
            i += add_word(buffer, "##start", i);
        if (end && my_strcmp(room[j][0], robot->room_end[0]) == 0)
