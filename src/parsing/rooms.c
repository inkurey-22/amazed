...
static int verify_link(robot_t *r, int indi)
{
    ...
    for (int i = r->room->indiceAllrooms - 1; i >= 0; i--) {
        if (my_strcmp(r->room->alR[i][0], r->room->link_s[indi]) == 0)
            room_s = true;
        if (my_strcmp(r->room->alR[i][0], r->room->link_e[indi]) == 0)
            room_e = true;
    }
    if (room_s && room_e)
        return (0);
...
int verify_names(robot_t *robot, char **telltale)
{
    for (int i = robot->room->indiceAllrooms - 1; i >= 0; i--) {
        if (my_strcmp(robot->room->alR[i][0], telltale[0]) == 0 ||
        (my_strcmp(robot->room->alR[i][1], telltale[1]) == 0
        && my_strcmp(robot->room->alR[i][2], telltale[2]) == 0)) {
            robot->index = "links";
